...
class OrderInvoiceDataSerializers(serializers.ModelSerializer):
    ...
    class Meta:
        model= Order
        fields=['id','order_type','created_at','invoice_date','invoice_no','po_num','term','due_date','ageing','open_balance']
...
class AdminAgeingReportSerializers(serializers.ModelSerializer):
    ...
    def get_invoice_data(self,obj):
        ...
        order_qs = Order.objects.filter(customer=obj,remaining_amount__gt=0.0).order_by('invoice_date')
        for obj in order_qs:
            today = datetime.now().date()
...
class OrderStatementSerializers(serializers.ModelSerializer):
    model_name = serializers.ReadOnlyField(default='Order')
    ...
    store_name = serializers.CharField(source='customer.store_name')
    balance = serializers.SerializerMethodField()

    class Meta:
        model = Order
        fields = ['id','model_name','amount','balance','transaction_data','transaction_date','customer_id','customer_name','store_name']

    def get_transaction_data(self,obj):
        value = "INV" + " #" + str(obj.invoice_no) + ".Due " + str(obj.due_date.strftime("%m-%d-%Y")) +  "."
        return value
    
    def get_transaction_date(self,obj):
        if obj.invoice_date:
            value =  obj.invoice_date
        else:
            value = ""
        return value
...
class PaymentStatementSerializers(serializers.ModelSerializer):
    model_name = serializers.ReadOnlyField(default='PaymentHistory')
    transaction_data = serializers.SerializerMethodField()
    transaction_date = serializers.SerializerMethodField()
    customer_id = serializers.CharField(source='customer.id')
    customer_name = serializers.CharField(source='customer.full_name')
    ...
    def get_transaction_date(self,obj):
        if obj.payment_date:
            value =  obj.payment_date
        else:
            value = ""
        return value
    
    def get_amount(self,obj):
        value = -obj.amount_recieved
        return value
...
class SalesStoreCreditsSerializers(serializers.ModelSerializer):
    ...
    class Meta:
        model = Customer
        fields = ['id','store_name','full_name','invoice_count','invoice_data','available_credit']

    def get_invoice_count(self,obj):
        value = Order.objects.filter(customer=obj,payment_status__in = ['PARTIAL','NOT_PAID']).count()
        return value

    def get_available_credit(self,obj):
        data_qs = CreditMemo.objects.filter(customer=obj)
        value = data_qs.aggregate(Sum('credit_amount'))['credit_amount__sum']
        return value

    def get_invoice_data(self,obj):
        request = self.context.get('request')
        order_qs = Order.objects.filter(customer=obj,payment_status__in = ['PARTIAL','NOT_PAID'])
        value = []
        for obj in order_qs:
            data = {
                'invoice_no' : obj.invoice_no,
                ...
                'due_date' : obj.due_date,
                'term,' : obj.term,
                'invoice_pdf' : request.build_absolute_uri(obj.invoice_pdf.url) if obj.invoice_pdf else "" ,
