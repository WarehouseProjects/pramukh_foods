...
class SalesStoreCredits(APIView):
    authentication_classes = (MyCustomAuth,)
    ...
    def post(self, request, format=None):
        ...
        if request.user.id == 13:
            store_qs = Customer.objects.all().order_by('store_name')
        else:
            store_qs = Customer.objects.filter(sales_person=request.user).order_by('store_name')
        ...
        total_record = len(store_qs)
        list_with_pagelimit = store_qs[start:end]
        filter_record = len(list_with_pagelimit)

        serializer = SalesStoreCreditsSerializers(list_with_pagelimit, many=True,context={'request':request})
